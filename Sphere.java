...
public class Sphere extends Surface {
    ...
    public boolean intersect(IntersectionRecord outRecord, Ray ray) {
    	...
    	Vector3 d = new Vector3(center.x-ray.origin.x,center.y-ray.origin.y,center.z-ray.origin.z);
    	
    	double l = Math.sqrt(d.squaredLength());
    	
    	d.normalize();
    	ray.direction.normalize();
    	
    	double costheta = d.dot(ray.direction);
    	double t;
    	
    	if (l > radius){
    		double coslimit = Math.sqrt(l*l-radius*radius)/l;
    		if (coslimit>costheta)
    			return false;
    		t = l*costheta-Math.sqrt(l*l*costheta*costheta-l*l+radius*radius);
    	}
        ...
        outRecord.frame.o.set(ray.origin.x+t*ray.direction.x,ray.origin.y+t*ray.direction.y,ray.origin.z+t*ray.direction.z);

        outRecord.frame.w.set((ray.origin.x+t*ray.direction.x-center.x)/radius, (ray.origin.y+t*ray.direction.y-center.y)/radius, (ray.origin.z+t*ray.direction.z-center.z)/radius);
